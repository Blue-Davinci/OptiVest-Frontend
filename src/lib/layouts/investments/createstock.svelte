<script>
    ...
    import {stockSchema} from '$lib/settings/schema.js';
    ...
    const form = superForm(data.stockForm, {
		validators: zodClient(stockSchema),
		dataType: 'json',
        invalidateAll: true,
		onUpdated({ form }) {
            console.log(form);
			if (form.message && form.message.success) {
				...
				console.log('Stock created:', form.message.data);
				isDrawerOpen = false;
...
<Dialog.Root bind:open={isDrawerOpen}>
	<Dialog.Content class="p-6">
		...
		<form method="POST" action="?/stockinvestment" use:enhance>
			<Form.Field {form} name="stock_symbol">
				<Form.Control let:attrs>
					<Form.Label>Stock Symbol</Form.Label>
					<Input
						{...attrs}
						placeholder="Enter the stock symbol"
						bind:value={$formData.stock_symbol}
			...
			<Form.Field {form} name="dividend_yield">
				<Form.Control let:attrs>
					...
					<Input
						...
						bind:value={$formData.dividend_yield}
						class="block w-full rounded border-gray-300 p-2 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50"
					/>
				</Form.Control>
				<Form.FieldErrors />
