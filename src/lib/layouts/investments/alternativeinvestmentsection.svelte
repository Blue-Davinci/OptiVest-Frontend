...
<div class="min-h-screen space-y-6 p-6">
	...
	<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
		<div
			class="relative overflow-hidden rounded-xl border border-gray-100 bg-white p-6 shadow-sm transition-all hover:shadow-md dark:border-gray-700 dark:bg-gray-800"
		>
			<div class="absolute right-0 top-0 h-24 w-24 -translate-y-8 translate-x-8 transform">
			...
			<div class="relative">
				...
				<p
					...
					title={formatCurrency(totalPortfolioValue)}
				>
		...
		>
			...
			<div class="relative">
				...
				<p
					class="mt-2 truncate text-2xl font-bold text-gray-900 dark:text-white"
					title={formatCurrency(totalAnnualRevenue)}
				>
		...
		<div
			class="relative overflow-hidden rounded-xl border border-gray-100 bg-white p-6 shadow-sm transition-all hover:shadow-md dark:border-gray-700 dark:bg-gray-800 sm:col-span-2 lg:col-span-1"
		>
			<div class="absolute right-0 top-0 h-24 w-24 -translate-y-8 translate-x-8 transform">
				<div class="absolute inset-0 bg-purple-50 opacity-50 dark:bg-purple-900/10"></div>
			</div>
			<div class="relative">
				...
				<p
					class="mt-2 truncate text-2xl font-bold text-gray-900 dark:text-white"
					title={formatPercentage(averageProfitMargin / 100)}
				>
	...
	{#if alternativeInvestments.length > 0}
		<section
			class="rounded-xl border border-gray-100 bg-white p-6 shadow-lg transition-all hover:shadow-xl dark:border-gray-700 dark:bg-gray-800"
		>
			<!-- Header -->
			...
			<div class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
				{#each enhancedFilter() as alt}
					<div
						class="group relative rounded-xl border border-gray-200 bg-gray-50 p-6 transition-all hover:-translate-y-1 hover:shadow-lg dark:border-gray-600 dark:bg-gray-700/50"
					>
						<div class="mb-4 flex items-center justify-between">
							...
							<span
								class="rounded-full bg-purple-100 px-3 py-1 text-sm text-purple-600 dark:bg-purple-900/50"
							>
							...
							>
								<p class="flex items-center gap-2">
									...
									<span class="text-sm text-gray-600 dark:text-gray-300"
										>{alt.alternative.investment_type}</span
									>
								</p>
								<p class="flex items-center gap-2">
									<MapPin size={16} class="text-gray-400" />
									<span class="text-sm text-gray-600 dark:text-gray-300"
										>{alt.alternative.location}</span
									>
								</p>
							</div>
	...
	{:else}
		<div
			class="flex flex-col items-center justify-center rounded-xl border border-gray-100 bg-white p-12 text-center shadow-sm dark:border-gray-700 dark:bg-gray-800"
		>
